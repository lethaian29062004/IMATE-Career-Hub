...
function Calendar({
  ...
  return (
    <DayPicker
      ...
      className={cn(
        "bg-background group/calendar p-3 [--cell-size:theme(spacing.8)]", // Sửa lại biến CSS nếu dùng v4
      ...
      classNames={{
        // ... giữ nguyên các classNames của bạn ...
...
// Tách riêng để xử lý ref sạch sẽ hơn
function CalendarDayButton(props: React.ComponentProps<typeof DayButton>) {
  ...
  return (
    <Button
      variant="ghost"
      size="icon"
      // react-day-picker v9 truyền ref qua buttonProps, không cần tự tạo useRef riêng trừ khi bạn muốn can thiệp sâu
      ...
      className={cn(
        ...
        "min-w-[var(--cell-size)]", // Cách viết an toàn cho cả Tailwind v3 và v4
